...
│   ├── app/
│   │   └── index.ts           # Express app (middleware, routes, Swagger)
│   ├── config/
...
│   ├── database/
│   │   ├── connection.ts
│   │   └── models/            # User, RefreshToken, AuditLog, Event
│   ├── shared/
│   │   ├── middleware/        # authenticate (Bearer JWT)
│   │   ├── dto/               # Shared schema types for OpenAPI
│   │   └── utils/             # JWT, tokens, lockout, audit, etc.
│   └── scripts/
│       └── generate-openapi.ts
...
2. From `backend`: `npm run openapi`.
